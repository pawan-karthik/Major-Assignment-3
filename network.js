function simulate(data, svg) {
    ...
    data.forEach((article, index) => {
        ...
        // Create a map to associate each author with their affiliation
        const authorAffiliationsMap = {};
        affiliationsList.forEach((aff) => {
            if (aff) {
                const parts = aff.split(',').map(item => item.trim());
                // If there's at least one part, treat the first part as the name, and the rest as affiliation
        ...
        authorNames.forEach(author => {
            if (!authors[author]) {
                //author = author.split(' ').map(name => name.trim())[0];
                //console.log(author_parts[0]);
                //.log(author,authorAffiliationsMap,authorAffiliationsMap[author]);kill();
                authors[author] = { 
                    name: author,
                    affiliations: authorAffiliationsMap[author] || 'Unknown affiliation',  // Proper affiliation mapping
                    group: index,
                    country: country
                };
    ...
    // Get top 10 countries by publication count
    ...
    const node_elements = main_group.append("g")
        ...
        .on("click", function (event, d) {
            tooltip.style("visibility", "visible")
                .html(`Author: ${d.name}<br>Authors with affiliations: ${d.affiliations}`);  // Show affiliation in tooltip
            tooltip.style("top", (event.pageY - 10) + "px")
                .style("left", (event.pageX + 10) + "px");
        });
    ...
    controls.append("label").text("ForceCollide: ");
    controls.append("input")
        .attr("type", "range")
        .attr("min", 1)
        .attr("max", 5)
